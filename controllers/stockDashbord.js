...
AND req_date BETWEEN current_date::date - INTERVAL '30 days' AND current_date`
  );
...
from PUR_STORE_REQ_HDR  
where marked is null AND company_code = ${req.user.company}
AND unit_code = ${req.user.unit} AND fin_year = '${req.user.finyear}'
AND req_date BETWEEN current_date::date - INTERVAL '1 year' AND current_date`
  );
...
WHERE
 ...
 WHERE 
     ...
     AND i.item_code IN (
         ...
         FROM t_stock_trans 
         WHERE marked IS NULL
          AND trans_dt BETWEEN current_date::date - INTERVAL '30 days' AND current_date
     ) limit 10`
  );
  const formattedData = allItemWithAmount.rows.map((row) => ({
    ...row,

    item: row.item, // Convert qty to number
