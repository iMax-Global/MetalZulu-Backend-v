...
exports.getAllSauda = wrapper(async (req, res, next) => {
  ...
  const sauda = await client.query(
    `SELECT SAUDA_CODE contract_code, timestamptostring(SAUDA_DATE) contract_date, Get_external_entity(dealer_code)DEALER, 
    get_distributor(cust_code)CUSTOMER ,get_item_category(item_cat_code) ITEM_CATEGORY, 
    get_item(item_code)ITEM,get_quality(SAUDA_QUALITY) Grade,  SAUDA_RATE contract_rate, SAUDA_QTY contract_qty,
    get_freight(freight_type_code)FREIGHT_TYPE
    FROM sl_trans_sauda_hdr where marked is null and company_code= ${req.user.company} and fin_year='${req.user.finyear}' and unit_code=${req.user.unit}`
  );
  res.status(200).json({
    status: "success",
    data: {
...
exports.createSauda = wrapper(async (req, res, next) => {
  ...
  const response1 = await client.query(
    `SELECT MAX(COALESCE(CAST(SUBSTR(SAUDA_CODE, 8) AS INT), 0)) AS M 
  ...
  for (let i = 0; i < arr.length; i++) {
    if (req.body[arr[i].responseFieldName]) {
      if (!arr[i].typeArray) {
        ...
        const query = `INSERT INTO ${arr[i].tableName} (SAUDA_CODE, ${fields}, company_code, user_code, unit_code, fin_year) VALUES ('${saudaCode}', ${values}, '${req.user.company}', '${req.user[0].spec_code}', '${req.user.unit}', '${req.user.finyear}')`;
        // console.log(query);
...
exports.deleteSauda = wrapper(async (req, res, next) => {
  ...
  const tableArray = [
    ...
    "sl_trans_booking_size_detail"
  ]; // List of tables to check
  const result = await Promise.all(
    tableArray.map(async (table) => {
      return await client.query(
        `select ref_TABLE_C('${table}','sauda_code','${req.params.code}')`
      );
    })
  );
// SELECT ref_table('${table}', 'item_code', '${req.params.code}')
  ...
  if (isAnyTableUsed) {
    ...
    return res.status(200).json({
      status: "success",
      message: "Sauda is Already Tagged",
      isUsed: true,
    });
  } else{
  for (let i = 0; i < arr.length; i++) {
