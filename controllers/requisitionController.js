...
const jsonData = JSON.parse(
  fs.readFileSync(`${__dirname}/../requisition.json`, "utf8")
);
...
//     `SELECT MAX(CAST(req_code AS INT)) AS M
...
const generateReqId = async (client) => {
  const response1 = await client.query(
    `SELECT MAX(CAST(substring(req_code from '[0-9]+$') AS INTEGER)) AS M FROM PUR_STORE_REQ_HDR`
  );
  // console.log("ggjhjkkjkj", response1);
...
exports.getAllRequisition = wrapper(async (req, res, next) => {
  ...
  const Requisition = await client.query(
    `select req_code, timestamptostring(req_date) req_date, req_time,get_department(dept_cd)department,get_employee(emp_cd)employee from PUR_STORE_REQ_HDR where MARKED is null and company_code= ${req.user.company} and fin_year='${req.user.finyear}'  and unit_code =${req.user.unit}`
  );
  res.status(200).json({
    status: "success",
    data: {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].lovFields) {
      ...
      for (const key in obj) {
        ...
        console.log(query, "data additional______________________");
        const dbData = await client.query(query);
        data[key] = dbData;
      }
    }
  ...
  for (let i = 0; i < arr.length; i++) {
    if (req.body[arr[i].responseFieldName]) {
      if (!arr[i].typeArray) {
        ...
        //   });
        // }
        const query = `INSERT INTO ${arr[i].tableName} (Req_code,  ${fields}, company_code, user_code, unit_code, fin_year, status, company_type) VALUES ('${openingBalanceCode}',  ${values}, '${req.user.company}', '${req.user[0].spec_code}', '${req.user.unit}', '${req.user.finyear}', 'M', 'R')`;
        // console.log(query);
        ...
        for (let j = 0; j < arr1.length; j++) {
          ...
          fields = fields.slice(0, -2);
          values = values.slice(0, -2);
          const query = `INSERT INTO ${arr[i].tableName} (${arr[i].uniqueReqIdentifier}, ${fields}, company_code, user_code, unit_code, fin_year, status) VALUES ('${openingBalanceCode}', ${values}, '${req.user.company}', '${req.user[0].spec_code}', '${req.user.unit}', '${req.user.finyear}', 'M' )`;
          console.log(query);
          await client.query(query);
        }
      }
...
exports.deleteOpening = wrapper(async (req, res, next) => {
  ...
  const arr = jsonData.getNdelete.dataSources;
  const tableArray = [
    "pur_trans_ingot_issue_hdr",
    "pur_trans_ingot_issue",
  ...
  const result = await Promise.all(
    tableArray.map(async (table) => {
      return await client.query(
        `SELECT ref_table_c('${table}', 'req_code', '${req.params.code}')`
      );
    })
  );
