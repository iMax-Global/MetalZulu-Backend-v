...
//   fs.readFileSync(`${__dirname}/../dispatchPlanning.json`, "utf8")
// );
...
exports.getAllDailyPlan = wrapper(async (req, res, next) => {
  const client = req.dbConnection;

  const order = await client.query(
    `select 
    sales_return_code return_code,
    timestamptostring(sales_return_date) return_date,
    invoice_no,
    get_distributor(distributor_code) customer_name,
    get_external_entity(dealer_code) dealer_name
    from sl_trans_sale_return_hdr where marked is null and company_code= ${req.user.company} and fin_year='${req.user.finyear}'  and unit_code =${req.user.unit}`
  );
...
exports.getAllDailyPlanHdr = wrapper(async (req, res, next) => {
  const client = req.dbConnection;

  const order = await client.query(
    `
    SELECT  H.INVOICE_NO, timestamptostring(H.INVOICE_DATE) invoice_date,
       GET_DISTRIBUTOR(H.DISTRIBUTOR_CODE) AS distributor_name,
       GET_DEALER_NAME(H.DEALER_CODE) AS dealer_name,
       get_del_site(H.DEL_ADD) delivery_site, (coalesce(SUM(D.TOTAL_QTY), 0)-coalesce(SUM(SI.RETN_WT),0))PENDING_WT   
FROM SL_TRANS_INVOICE_HDR H
JOIN sl_trans_inv_size_detail D ON (H.INVOICE_NO = D.INVOICE_NO)
LEFT JOIN  (SELECT coalesce(sum(RET_WT), 0)RETN_WT,INVOICE_NO,INVOICE_UNIQ
         FROM SL_SALE_RETURN_SIZE_DETAIL
         WHERE MARKED IS NULL
      group by INVOICE_NO,INVOICE_UNIQ) SI ON H.INVOICE_NO = SI.INVOICE_NO
WHERE H.MARKED IS NULL
 AND H.UNIT_CODE = ${req.user.unit} AND H.FIN_YEAR = '${req.user.finyear}'  and h.company_code =${req.user.company} 
 group by H.INVOICE_DATE,     
       H.DEL_ADD,    
       H.INVOICE_NO,
       H.DISTRIBUTOR_CODE,
       H.DEALER_CODE  
       having (coalesce(SUM(D.TOTAL_QTY), 0)-coalesce(SUM(SI.RETN_WT),0))>0 `
  );
...
exports.getdetailsOfPlanitem = wrapper(async (req, res, next) => {
  const client = req.dbConnection;
  // console.log(req.body);
  // console.log(req.body);

  const headerDetail = await client.query(`SELECT H.GRN_NO AS REF_INVOICE,
  timestamptostring(H.INVOICE_DATE) invoice_date,
  H.invoice_type,
...
FROM SL_TRANS_INVOICE_HDR H
WHERE MARKED IS NULL 
and company_code= ${req.user.company} and fin_year='${req.user.finyear}'
AND CANCEL1 IS NULL
 and invoice_no = '${req.params.code}'`);
  ...
  const custdetail = await client.query(
    ...
    D.total_QTY,(COALESCE(D.total_QTY, 0) - COALESCE(SI.RETN_WT, 0))pending_qty,
    D.rate1,
    D.amount2,
    D.INVOICE_NO  
FROM SL_TRANS_INVOICE_HDR H
JOIN SL_TRANS_INV_SIZE_DETAIL D 
    ON H.INVOICE_NO = D.INVOICE_NO
    LEFT JOIN (
            SELECT COALESCE(SUM(RET_WT), 0) AS RETN_WT, INVOICE_NO, INVOICE_UNIQ
            FROM SL_SALE_RETURN_SIZE_DETAIL
            WHERE MARKED IS NULL
            GROUP BY INVOICE_NO, INVOICE_UNIQ
        ) SI ON D.INVOICE_NO = SI.INVOICE_NO and d.uniq_code=SI.INVOICE_UNIQ
WHERE
    H.marked is null and D.MARKED IS NULl
and (COALESCE(D.total_QTY, 0) - COALESCE(SI.RETN_WT, 0))>0
     and h.company_code= ${req.user.company} and h.fin_year='${req.user.finyear}'
       AND H.UNIT_CODE = ${req.user.unit} 
     and h.invoice_no = '${req.params.code}'`
  );
  res.status(200).json({
    status: "success",
    data: {
...
exports.getAllDispatch = wrapper(async (req, res, next) => {
  const client = req.dbConnection;
  ...
  const dispatch = await client.query(
    `Select
      Ref_Booking,
Booking_Code,
timestamptostring (booking_date::timestamp)booking_date,
...
ThickNESS,
Length,
get_masking(MASKING_CODE) masking,
MASKING_CODE,
get_item(Item_Code) item,
Item_Code,
get_texture(TEXTURE_Code) texture,
TEXTURE_Code,
get_color(COLOR_CODE) color,
COLOR_CODE,
Pcs, QTY, BAL_PCS, BAL_QT,
Sheet_Area,
Per_Sqm,
SHEET_PER_ROLL,
Thick_Pack,
...
const generateDailyProductionid = async (client) => {
  //const response = await client.query(`SELECT MAX(BOOKING_CODE) AS MAX FROM SL_TRANS_BOOKING_HDR`);

  //const response1 = await client.query(`SELECT MAX(TO_NUMBER(SUBSTR(BOOKING_CODE,8))) M FROM SL_TRANS_BOOKING_HDR`);
  // const response1=await client.query(`SELECT max(TO_NUMBER(BOOKING_CODE,'"x"99999'))M FROM SL_TRANS_BOOKING_HDR`);
  const response1 = await client.query(
    `SELECT MAX(CAST(substring(sales_return_code from '[0-9]+$') AS INTEGER)) AS M FROM sl_trans_sale_return_hdr`
  );
  // console.log("ggjhjkkjkj", response1);
...
exports.createDaily = wrapper(async (req, res, next) => {
  const client = req.dbConnection;
  const dispatchCode = await generateDailyProductionid(client);
  // // console.log(dispatchCode);
  // // console.log(dispatchCode);

  // console.log(req.body);

  await client.query(
    ` INSERT INTO sl_trans_sale_return_hdr (sales_return_code, invoice_no, distributor_code, dealer_code,  delivery_add, invoice_date,  sales_return_date, company_code, user_code, unit_code, fin_year) VALUES ('${dispatchCode}', '${
      req.body.bookingSize[0].invoice_no
    }',  ${req.body.bookingSize[0].distributor_code},  ${
      req.body.bookingSize[0].dealer_code
    },   ${req.body.bookingSize[0].del_add},'${formatDate(
      req.body.bookingSize[0].invoice_date
    )}', '${req.body.bookingSize[0].return_date}','${req.user.company}', '${
      req.user[0].spec_code
    }', '${req.user.unit}', '${req.user.finyear}')`
  );

  // Insert into dispatch_det table for each item in DispatchDetail
  for (let i = 0; i < req.body.DailyProductionDetail.length; i++) {
    const queryString = `INSERT INTO sl_sale_return_size_detail (item_code, size_code, quality, ret_wt, f_rate,amount, 
      invoice_no, sales_return_code, invoice_uniq, company_code, user_code, unit_code, fin_year) VALUES (  
         ${req.body.DailyProductionDetail[i].item_code} , ${req.body.DailyProductionDetail[i].size_code}, ${req.body.DailyProductionDetail[i].quality_code},
      ${req.body.DailyProductionDetail[i].return_qty}, ${req.body.DailyProductionDetail[i].rate1}, ${req.body.DailyProductionDetail[i].amount2}, 
      '${req.body.DailyProductionDetail[i].invoice_no}' ,'${dispatchCode}', 
      '${req.body.DailyProductionDetail[i].uniq_code}', '${req.user.company}', '${req.user[0].spec_code}', '${req.user.unit}', '${req.user.finyear}')`;
...
exports.deleteReturn = wrapper(async (req, res, next) => {
  ...
  await client.query(
    `Update sl_trans_sale_return_hdr SET MARKED='D' WHERE  sales_return_code='${req.params.code}'`
  );

  await client.query(
    `Update sl_sale_return_size_detail SET MARKED='D' WHERE  sales_return_code='${req.params.code}'`
  );
...
exports.getHsnForItem = wrapper(async (req, res, next) => {
  ...
  const size = await client.query(
    `select texture_code, get_texture(texture_code)texture_nm from sl_mst_item_text_det where item_code=${req.params.code}`
  );

  const grade = await client.query(
    `select color_code, get_color(color_code) from  sl_mst_item_color_det where item_code=${req.params.code}`
  );
  const density = await client.query(
    `select density  from sl_mst_item_text_det where item_code=${req.params.code}`
  );
...
exports.getItemValue = wrapper(async (req, res, next) => {
  const client = req.dbConnection;
  const { length, width, thickness, item_code, texture_code } = req.query;
  // // console.log(req.query.length); // Corrected typo
  ...
  const size = await client.query(
    "SELECT procedure_sheet_per_roll($1::integer, $2::integer, $3::numeric, $4::numeric, $5::numeric) AS result",
    [
      parseInt(item_code),
      parseInt(texture_code),
      parseFloat(length),
      parseFloat(width),
  ...
  res.status(200).json({
    status: "success",
    data: {
      size: size.rows[0].result,
    },
  });
});

exports.getOrderAmount = wrapper(async (req, res, next) => {
  const client = req.dbConnection;
  const { length, billing_uom, no_pcs, item_code, rate } = req.query;
  // // console.log(req.query.length); // Corrected typo
  console.log("length:", length);
  // console.log(req.query.length); // Corrected typo
  console.log("length:", length);
  ...
  const size = await client.query(
    "SELECT ORDER_AMOUNT_VALUE($1::integer, $2::integer, $3::numeric, $4::integer, $5::numeric) AS result",
    [
      parseInt(item_code),
      parseInt(billing_uom),
      parseFloat(length),
      parseFloat(no_pcs),
      parseFloat(rate),
    ]
...
exports.getTotalSqm = wrapper(async (req, res, next) => {
  const client = req.dbConnection;
  ...
  const size = await client.query(
    "SELECT total_sqmf_value($1::integer, $2::integer, $3::numeric, $4::numeric, $5::numeric, $6::integer) AS result",
    [
      parseInt(billing_uom),
      parseFloat(no_pcs),
      parseFloat(length),
