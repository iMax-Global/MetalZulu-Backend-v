...
const jsonData = JSON.parse(
  fs.readFileSync(`${__dirname}/../cityMaster.json`, "utf8")
);
...
const generateCityId = async (client) => {
  ...
  const response1 = await client.query(
    `SELECT MAX(city_code)M FROM sl_mst_city`
  );
...
exports.getAllCity = wrapper(async (req, res, next) => {
  ...
  const city = await client.query(
    `SELECT city_code, city_name, get_country(country_code) country_name,
    get_state(state_code) state_name, distt FROM sl_mst_city where marked is null and company_code='${req.user.company}' `
  );
  res.status(200).json({
    status: "success",
    data: {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (req.body[arr[i].responseFieldName]) {
      if (!arr[i].typeArray) {
        ...
        const query = `INSERT INTO ${arr[i].tableName} (CITY_CODE, ${fields}, company_code) VALUES (${cityCode}, ${values}, '${req.user.company}')`;
        console.log(query);
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].lovFields) {
      ...
      for (const key in obj) {
        ...
        if (key === "STATE_CODE" ) {
          // Filter by company_code only for STATE_CODE
          ...
          // Default without company_code filter
