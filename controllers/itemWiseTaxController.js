...
exports.getTaxData = wrapper(async (req, res, next) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    query += ` WHERE ${arr[i].uniqueTaxIdentifier}='${req.params.code}' and marked is null`;
    // console.log(query);
  ...
  for (let i = 0; i < arr.length; i++) {
    if (req.body[arr[i].responseFieldName]) {
      if (!arr[i].typeArray) {
        ...
        const query = `INSERT INTO ${arr[i].tableName} (tax_CODE, ${fields}, company_code, user_code, unit_code) VALUES ('${taxCode}', ${values}, '${req.user.company}', '${req.user[0].spec_code}', '${req.user.unit}')`;
        console.log(query);
        ...
        for (let j = 0; j < arr1.length; j++) {
          ...
          fields = fields.slice(0, -2);
          values = values.slice(0, -2);
          const query = `INSERT INTO ${arr[i].tableName} (${arr[i].uniqueTaxIdentifier}, ${fields}, company_code, user_code, unit_code) VALUES ('${taxCode}', ${values} , '${req.user.company}', '${req.user[0].spec_code}', '${req.user.unit}')`;
          // console.log(query);
          await client.query(query);
        }
      }
