...
const TotalUsers = await client.query(
  `select count(*) from sl_sec_spec_item_hdr where marked is null and company_code=${req.user.company}
   and get_employee_nm(spec_code) is not null`
);



const TotalUsersValue = TotalUsers.rows[0].count;

const TotalSites = await client.query(`select count(*) from sl_mst_site where marked is null and company=${req.user.company}`);

const TotalSitesValue = TotalSites.rows[0].count;




const UsersList = await client.query(`select spec_code, User_code,get_employee_nm(spec_code)User_name from sl_sec_spec_item_hdr where marked is null
and get_employee_nm(spec_code) is not null and company_code=${req.user.company} 
`);

console.log(UsersList,"total users list are these");


const SitesList = await client.query(`select site_code unit_code,site_desc unit_name,add1 unit_address,ph1 phone_no,pan_no,service_tax_no gst_no 
,pin_code pin_no,get_city(city_code)city_name
from sl_mst_site where marked is null and company =${req.user.company}`)
...
exports.getModules = wrapper(async (req, res, next) => {
  const client = req.dbConnection;
  const spec_code = req.params.spec_code; // Extract spec_code from route parameters
  console.log(spec_code, "spec-----------code");

  // Use parameterized query to prevent SQL injection
  const moudlelist = await client.query(
    `SELECT module, GET_MODULE(module) 
     FROM sl_sec_spec_item_det 
     WHERE MARKED IS NULL 
       AND SPEC_CD = $1 
       AND company_code = $2`,
    [spec_code, req.user.company]
  );
...
exports.getForms = wrapper(async (req, res, next) => {
  const client = req.dbConnection;
  const moduleId = req.params.module_id; // Extract spec_code from route parameters
  console.log(moduleId, "module-----------Id");

  // Use parameterized query to prevent SQL injection
  const Formlist = await client.query(
    `select FORM_ID , get_form_nm(FORM_ID)FORM_NAME 
from sl_sec_login_rights where MARKED is null
and company_code =${req.user.company} and MODULE_ID=${moduleId}`
  );
