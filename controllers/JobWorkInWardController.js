...
const jsonData = JSON.parse(
  fs.readFileSync(`${__dirname}/../JobWorkInward.json`, "utf8")
);
...
const generateReqId = async (client) => {
  const response1 = await client.query(
    `SELECT MAX(CAST(substring(ppc_code from '[0-9]+$') AS INTEGER)) AS M FROM ppc_job_in_dis_hdr`
  );
  // console.log("ggjhjkkjkj", response1);
...
exports.getAllRequisition = wrapper(async (req, res, next) => {
  ...
  const query = `select ppc_code job_code,   chalan_no, timestamptostring(ppc_date) job_date,   mode_of_trans,
...
exports.getReqData = wrapper(async (req, res, next) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    console.log(query,"query for jobinward");
...
exports.getAdditionalData = wrapper(async (req, res, next) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].lovFields) {
      ...
      for (const key in obj) {
        ...
        console.log(query,"additional data is this ");
        const dbData = await client.query(query);
        data[key] = dbData;
      }
    }
  ...
  const outwardCodeQuery = `
  SELECT H.PPC_CODE
    FROM  PPC_JOB_DIS_HDR H
    ...
    AND coalesce(H.PPC_CODE,'X') NOT IN (
      SELECT coalesce(out_job_no,'X')
      FROM PPC_JOB_IN_DIS_HDR
      WHERE MARKED IS null and unit_code=${req.user.unit}  and company_code=${req.user[0].company_code} and fin_year='${req.user.finyear}')
`;
try {
  console.log("-------------inwardCodeQuery-------------------",outwardCodeQuery);
  const outwardCodeData = await client.query(outwardCodeQuery);
  data.outward_code = outwardCodeData; // Add the result to the data object
  // console.log(data,"data is this -----------");
} catch (error) {
  console.error("Error fetching outward_code data:", error);
...
exports.fetchOutwardDetails = async (req, res) => {
  try {
    const client = req.dbConnection;
    const { outward_code } = req.query;
...
D.TOTAL,D.RATE,D.BURNING,d.hsn,d.uom_code,get_uom(d.uom_code)uom,  d.total_2,
(COALESCE(D.TOTAL,0)-(SELECT  COALESCE(SUM(COALESCE(q.TOTAL,0)),0)  FROM PPC_JOB_IN_DIS_HDR p, PPC_JOB_IN_DIS_DET q
WHERE p.PPC_CODE=q.PPC_CODE AND p. MARKED IS NULL AND q.MARKED IS NULL  AND q.ppc_out=H.PPC_CODE    
AND q.outward_s_no=d.uniq_code and p.unit_code=${req.user.unit}  and p.company_code=1 and p.fin_year='${req.user.finyear}'))NET_WT1,
D.ITEM_CODE,D.SIZE_CODE,D.GRADE,H.PPC_CODE
FROM PPC_JOB_DIS_HDR H,PPC_JOB_DIS_DET D
WHERE H.PPC_CODE=D.PPC_CODE AND H.MARKED IS NULL AND D.MARKED IS NULL and h.unit_code=${req.user.unit}  and
h.company_code=${req.user[0].company_code} and h.fin_year='${req.user.finyear}' and H.PPC_CODE='${outward_code}'`
    console.log("--------------------------------------");
    console.log(query,"fetch Outward query");
    console.log("--------------------------------------");
    const OutwardItemDetails = await client.query(query);
    
    // Return filtered customers as JSON response
    res.status(200).json(OutwardItemDetails);
  } catch (error) {
...
exports.getAdditionalDataVendor = wrapper(async (req, res, next) => {
  ...
  WHERE MARKED IS NULL AND jobtype like 'JBIN'`);
  ...
  for (let i = 0; i < arr.length; i++) {
    if (req.body[arr[i].responseFieldName]) {
      if (!arr[i].typeArray) {
        ...
        console.log(query,"query for update1");
        await client.query(query);
      } else {
        const arr1 = req.body[arr[i].responseFieldName];
        for (let j = 0; j < arr1.length; j++) {
          ...
          if (obj.PARAM === "UPDATE") {
            ...
            console.log(query,"query for insert in update");
            await client.query(query);
          }
        }
      }
