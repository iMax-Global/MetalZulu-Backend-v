...
const jsonData = JSON.parse(
  fs.readFileSync(`${__dirname}/../purchaserequisition.json`, "utf8")
);
...
//     `SELECT MAX(RQ_CODE)M FROM pur_rm_requisition_hdr`
//   );
...
const generateReqId = async (client) => {
  const response1 = await client.query(
    `SELECT MAX(CAST(substring(RQ_CODE from '[0-9]+$') AS INTEGER)) AS M FROM pur_rm_requisition_hdr`
  );
  // console.log("ggjhjkkjkj", response1);
...
exports.getAllRequisition = wrapper(async (req, res, next) => {
  ...
  const Requisition = await client.query(
    `select rq_code indent_code, timestamptostring(plan_date)  indent_date, timestamptostring(requirement_date)  requirement_date, get_department(dept_code) department, get_employee(requester_code) employee from pur_rm_requisition_hdr where marked is null and company_code= ${req.user.company} and fin_year='${req.user.finyear}'  and unit_code =${req.user.unit}`
  );
  res.status(200).json({
    status: "success",
    data: {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (req.body[arr[i].responseFieldName]) {
      if (!arr[i].typeArray) {
        ...
        //   });
        // }
        const query = `INSERT INTO ${arr[i].tableName} (Rq_code,  ${fields}, company_code, user_code, unit_code, fin_year) VALUES ('${openingBalanceCode}',  ${values}, '${req.user.company}', '${req.user[0].spec_code}', '${req.user.unit}', '${req.user.finyear}')`;
        // console.log(query);
...
exports.deleteOpening = wrapper(async (req, res, next) => {
  ...
  const arr = jsonData.getNdelete.dataSources;
  const tableArray = [
    { table: "pur_rmdeal_hdr", column: "req_code" },
    { table: "pur_rmdeal_det", column: "rq_code" },
  ]; // List of tables and their corresponding column names
  ...
  if (isAnyTableUsed) {
    ...
    return res.status(200).json({
      status: "success",
      message: "Requisition is  Already Tagged",
      isUsed: true,
    });
  } else {
  for (let i = 0; i < arr.length; i++) {
...
exports.OpeningValue = wrapper(async (req, res, next) => {
  const client = req.dbConnection;
  console.log("krrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrlooooooooooooooooooooooo");
  const {
    item_code,
    quality_code,
    size_code,
