...
where marked is null and company_code=${req.user.company} and unit_code=${req.user.unit} and item_code not in (select item_code from sl_mst_item where marked is null and item_category=1 and company_code=${req.user.company})
group by item_code`
  );
  ...
  const allSizeWithAmount = await client.query(
    ...
    where marked is null and company_code=${req.user.company} and unit_code=${req.user.unit} and item_cd in (select item_code from sl_mst_item where marked is null and item_category=1 and company_code=${req.user.company})
    group by item_cd`
  );
  ...
  const allGradeWithAmount = await client.query(
    ...
    where marked is null and company_code=${req.user.company} and unit_code=${req.user.unit}
    group by reason_code`
  );
  const formattedDataGrade = allGradeWithAmount.rows.map((row) => ({
    ...row,

    reasion: row.reasion, // Convert qty to number
    count: Number(row.count), // Convert amount to number
  }));
